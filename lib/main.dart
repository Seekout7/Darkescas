...
class AiNet {
  ...
  AiNet({required this.id, required this.charId, required this.x, required this.y, this.roomId = 0, this.speed = 1.0})
      : tx = x, ty = y, nextThink = 0, moving = false;
}
...
class EntityView {
  ...
  final Color color;
  EntityView({required this.id, required this.charId, required this.role, required this.x, required this.y,
    required this.room, required this.moving, required this.hidden, required this.inside, required this.color});
}

class Room { final String name; final Rect rect; Room(this.name, this.rect); }

class MapDef {
  final String name; final Rect office;
  final Offset leftDoor, rightDoor, vent, insideLeft, insideRight, insideVent;
  final List<Room> rooms;
  MapDef({required this.name, required this.office, required this.leftDoor, required this.rightDoor,
    required this.vent, required this.insideLeft, required this.insideRight, required this.insideVent, required this.rooms});
}

class CharDef {
  final String name; final Color color; final double speed, cooldown;
  final String active, passive;
  CharDef({required this.name, required this.color, required this.speed, required this.cooldown,
    required this.active, required this.passive});
}

class DiscoveredHost {
  final String addr; final int port; String session; int count, map; double last;
...
class _GameScreenState extends State<GameScreen> with SingleTickerProviderStateMixin {
  ...
  int page = 0; // 0 menu 1 discover 2 lobby 3 game 4 end
  bool isHost = false;
  int myId = -1, guardId = -1, myRole = -1, myChar = -1, currentMap = 0;
  String myName = "", status = "", endMsg = "";

  RawDatagramSocket? sock;
  InternetAddress? hostAddr; int hostPort = netPort;
  ...
  int noiseRoom = -1; double noiseUntil = 0;

  Offset localPos = Offset.zero;
  bool localMoving = false, myInside = false, myBoost = false;
  double myCooldown = 0;
  Offset joyThumb = Offset.zero; double joyX = 0, joyY = 0;
  ...
  @override
  void dispose() {
    _ticker?.dispose(); slowTimer?.cancel(); sock?.close(); ipCtrl.dispose();
    super.dispose();
  }
  ...
  void setQuality(Quality v) { quality = v; q = GQ.of(v); ui(); }
  ...
  Future<bool> startHostSocket() async {
    ...
    try {
      ...
      sock!.broadcastEnabled = true; sock!.listen(_onSock);
      return true;
    } catch (e) { status = "Host hatası: $e"; return false; }
  }
  ...
  Future<bool> startClientSocket() async {
    ...
    try {
      ...
      sock!.broadcastEnabled = true; sock!.listen(_onSock);
      return true;
    } catch (e) { status = "Socket hatası: $e"; return false; }
  }

  void _closeNet() { try { sock?.close(); } catch (_) {} sock = null; }
  ...
  void _onSock(RawSocketEvent e) {
    ...
    try {
      ...
      isHost ? _hostPacket(gs(m["t"]), p, dg) : _clientPacket(gs(m["t"]), p, dg);
    } catch (_) {}
  }
  ...
  void _toHost(Map<String, dynamic> d) { if (hostAddr != null) _send(d, hostAddr!, hostPort); }
  void _toAll(Map<String, dynamic> d) { if (!isHost) return; for (var e in endpoints.values) _send(d, e.address, e.port); }

  int _epId(InternetAddress a, int port) {
    for (var e in endpoints.entries) if (e.value.address.address == a.address && e.value.port == port) return e.key;
    return -1;
  }
  ...
  void _hostPacket(String t, Map<String, dynamic> p, Datagram dg) {
    ...
    if (t == "hello") {
      ...
      if (ex != -1) { _send({"t": "welcome", "p": {"id": ex}}, dg.address, dg.port); return; }
      if (page != 2 || players.length >= maxPlayers) { _send({"t": "err", "p": {"m": "Katılamazsın."}}, dg.address, dg.port); return; }
      int id = 1; while (players.any((x) => x.id == id)) id++;
      ...
      status = "Oyuncu katıldı."; _broadcastLobby();
      return;
    }
    int id = _epId(dg.address, dg.port); if (id == -1) return;
    PlayerNet? pl = _player(id); if (pl != null) pl.lastSeen = nowSec();
    if (t == "ping") return;
    ...
    if (t == "state" && page == 3 && pl != null && pl.role == 1 && pl.alive) {
      pl.x = gd(p["x"]); pl.y = gd(p["y"]); pl.moving = gb(p["mv"]);
  ...
  void _clientPacket(String t, Map<String, dynamic> p, Datagram dg) {
    ...
    if (t == "hostinfo" && page == 1) {
      ...
      DiscoveredHost? f; for (var d in discovered) if (d.addr == key) f = d;
      if (f == null) { f = DiscoveredHost(addr: key, port: dg.port, session: gs(p["name"]), count: gi(p["count"]), map: gi(p["map"]), last: nowSec()); discovered.add(f); }
      else { f.session = gs(p["name"]); f.count = gi(p["count"]); f.map = gi(p["map"]); f.last = nowSec(); }
      ui(); return;
    }
    if (t == "welcome") { myId = gi(p["id"]); page = 2; status = "Lobidesin."; ui(); return; }
    if (t == "lobby") {
      players.clear(); currentMap = gi(p["map"]);
      if (p["players"] is List) for (var it in (p["players"] as List)) {
        if (it is Map) { Map<String, dynamic> pl = it.cast<String, dynamic>();
  ...
  Future<void> hostGame() async {
    ...
    isHost = true; myId = 0; guardId = -1; myRole = -1; myChar = -1; currentMap = 0;
    players.clear(); endpoints.clear(); ais.clear(); remote.clear();
    players.add(PlayerNet(id: 0, name: myName, lastSeen: nowSec()));
    page = 2; status = "Host hazır."; _broadcastLobby(); ui();
  }

  Future<void> openDiscovery() async {
    if (!await startClientSocket()) { ui(); return; }
    isHost = false; myId = -1; discovered.clear(); page = 1;
    _sendDiscover(); lastDiscovery = nowSec(); ui();
  }

  void _sendDiscover() { try { _send({"t": "discover"}, InternetAddress("255.255.255.255"), netPort); } catch (_) {} }

  Future<void> joinIp() async {
    InternetAddress? a; try { a = InternetAddress(ipCtrl.text.trim()); } catch (_) {}
    if (a == null) { status = "Geçerli IP gir."; ui(); return; }
    if (!await startClientSocket()) { ui(); return; }
    isHost = false; myId = -1; hostAddr = a; hostPort = netPort;
    _hello(); page = 0; status = "Bağlanıyor..."; lastHostPacket = nowSec(); ui();
  }

  void _joinDisc(DiscoveredHost d) {
    try { hostAddr = InternetAddress(d.addr); hostPort = d.port; _hello(); page = 0; status = "Bağlanıyor..."; lastHostPacket = nowSec(); ui(); } catch (_) {}
  }
  ...
  void returnMenu() {
    ...
    _ticker?.dispose(); _ticker = null; _closeNet();
    isHost = false; page = 0; myId = -1; guardId = -1; myRole = -1; myChar = -1; winner = 0;
    players.clear(); endpoints.clear(); ais.clear(); remote.clear(); discovered.clear(); particles.clear();
    status = "Ana menü."; ui();
  }

  void _selectChar(int c) { if (page != 2) return; isHost ? _hostSelectChar(myId, c) : _toHost({"t": "char", "p": {"c": c}}); }

  void _hostSelectChar(int pid, int c) {
    if (page != 2 || c < 0 || c >= chars.length) return;
    PlayerNet? pl = _player(pid); if (pl == null) return;
    for (var o in players) if (o.id != pid && o.charId == c) { status = "Bu karakter seçili."; _broadcastLobby(); return; }
    pl.charId = c; status = "${chars[c].name} seçildi."; _broadcastLobby();
  }

  void _setMap(int m) { if (!isHost || page != 2) return; currentMap = m.clamp(0, 1); _broadcastLobby(); }
  ...
  void _startHost() {
    ...
    activeMap = makeMap(currentMap); _resetVars();
    List<int> ids = players.map((e) => e.id).toList();
    guardId = ids[rnd.nextInt(ids.length)];
    List<int> used = [];
    for (var p in players) {
      if (p.id == guardId) { p.role = 0; p.charId = -1; p.x = activeMap!.office.center.dx; p.y = activeMap!.office.center.dy; p.insideOffice = true; p.roomId = -2; }
      else {
        p.role = 1;
        if (p.charId < 0 || p.charId >= chars.length || used.contains(p.charId)) p.charId = _freeChar(used);
        used.add(p.charId);
        Offset s = _randSpawn(); p.x = s.dx; p.y = s.dy; p.insideOffice = false; p.roomId = _roomAt(s);
      }
      p.alive = true; p.moving = false; p.cooldownUntil = 0; p.boostUntil = 0; p.silentUntil = 0; p.lightUntil = 0; p.ventProgress = 0;
      if (p.id == myId) { myRole = p.role; myChar = p.charId; localPos = Offset(p.x, p.y); myInside = p.insideOffice; }
    }
    ais.clear(); int aiId = -100;
    for (int c = 0; c < chars.length; c++) if (!used.contains(c)) {
      Offset s = _randSpawn(); ais.add(AiNet(id: aiId--, charId: c, x: s.dx, y: s.dy, roomId: max(0, _roomAt(s)), speed: 1.0 + rnd.nextDouble() * 0.7));
    }
    _toAll({"t": "start", "p": {
      "map": currentMap, "guard": guardId,
    ...
    page = 3; _startTicker(); ui();
  }

  void _startFromPayload(Map<String, dynamic> p) {
    _resetVars();
    currentMap = gi(p["map"]); guardId = gi(p["guard"]); activeMap = makeMap(currentMap);
    players.clear(); remote.clear(); ais.clear();
    if (p["players"] is List) for (var it in (p["players"] as List)) {
      if (it is Map) { Map<String, dynamic> ps = it.cast<String, dynamic>();
        int id = gi(ps["id"]); int ch = gi(ps["char"]); int rl = gi(ps["role"]); double x = gd(ps["x"]), y = gd(ps["y"]);
        if (id == myId) { myRole = rl; myChar = ch; localPos = Offset(x, y); myInside = rl == 0; }
        else remote[id] = EntityView(id: id, charId: ch, role: rl, x: x, y: y, room: rl == 0 ? -2 : _roomAt(Offset(x, y)), moving: false, hidden: false, inside: rl == 0, color: _charColor(ch));
      }
    }
    if (p["ais"] is List) for (var it in (p["ais"] as List)) {
      if (it is Map) { Map<String, dynamic> a = it.cast<String, dynamic>();
        remote[gi(a["id"])] = EntityView(id: gi(a["id"]), charId: gi(a["char"]), role: 2, x: gd(a["x"]), y: gd(a["y"]), room: _roomAt(Offset(gd(a["x"]), gd(a["y"]))), moving: false, hidden: false, inside: false, color: _charColor(gi(a["char"])));
      }
    }
    page = 3; _startTicker(); ui();
  }
  ...
  void _resetVars() {
    gameTime = 0; energy = 100; leftDoorClosed = true; rightDoorClosed = true; flashOn = false; camOn = false; blackout = false; winner = 0; endMsg = "";
    forcedL = 0; forcedR = 0; dmgL = 0; dmgR = 0; camJam = 0; ctrlLock = 0; noiseRoom = -1; noiseUntil = 0;
    localMoving = false; myInside = myRole == 0; myCooldown = 0; myBoost = false; selectedCamRoom = 0;
    particles.clear(); shake = 0; jumpscare = 0;
  }

  void _startTicker() { _ticker?.dispose(); _ticker = createTicker(_tick)..start(); }
  ...
  void _tick(Duration d) {
    ...
    if (winner == 0) {
      ...
      if (isHost) _hostUpdate(dt); else if (myRole == 1 && nowSec() - lastStateSend > 0.1) _sendState();
    }
    _updateParticles(dt);
    shake = max(0, shake - dt * 3);
    jumpscare = max(0, jumpscare - dt);
    _frame.value++;
    _hudAccum += dt;
  ...
  void slowTick(Timer t) {
    double now = nowSec();
    if (page == 1) { if (now - lastDiscovery > 2) { lastDiscovery = now; _sendDiscover(); }
      int b = discovered.length; discovered.removeWhere((d) => now - d.last > 6); if (b != discovered.length) ui(); }
    ...
    if (isHost && (page == 2 || page == 3)) for (int i = players.length - 1; i >= 0; i--) {
  ...
  void _hostUpdate(double dt) {
    ...
    gameTime += dt;
    double drain = 0.08;
    if (leftDoorClosed) drain += 0.4; if (rightDoorClosed) drain += 0.4;
    if (flashOn) drain += 0.35; if (camOn) drain += 0.5;
    for (var p in players) if (p.role == 1 && p.alive && p.charId >= 0 && p.charId < chars.length && chars[p.charId].passive == "energy") {
      if ((Offset(p.x, p.y) - activeMap!.office.center).distance < 10) drain += 0.15;
    }
    energy -= drain * dt;
    if (energy <= 0 && !blackout) { energy = 0; blackout = true; leftDoorClosed = false; rightDoorClosed = false; flashOn = false; camOn = false; }
    ...
    if (me != null) { myInside = me.insideOffice; myCooldown = max(0, me.cooldownUntil - gameTime); myBoost = me.boostUntil > gameTime; }
    double now = nowSec();
  ...
  void _updateAI(double dt) {
    ...
    for (var a in ais) {
      ...
      if (dir.distance > 0.2) { Offset n = Offset(a.x, a.y) + (dir / dir.distance) * a.speed * dt; a.x = n.dx; a.y = n.dy; a.moving = true; }
      else a.moving = false;
    }
  }
  ...
  void _moveLocal(double dt) {
    ...
    Offset dir = Offset(joyX, joyY); localMoving = dir.distance > 0.1;
    if (localMoving) {
      if (dir.distance > 1) dir = dir / dir.distance;
      double sp = _localSpeed();
      Offset n = localPos + dir * sp * dt;
      if (_walkable(n)) localPos = n;
      else { Offset nx = localPos + Offset(dir.dx * sp * dt, 0); if (_walkable(nx)) localPos = nx;
        Offset ny = localPos + Offset(0, dir.dy * sp * dt); if (_walkable(ny)) localPos = ny; }
    }
    if (isHost) { PlayerNet? me = _player(myId); if (me != null) { me.x = localPos.dx; me.y = localPos.dy; me.moving = localMoving; me.roomId = _roomAt(localPos); } }
  }

  double _localSpeed() {
    double s = 3.2;
    if (myChar >= 0 && myChar < chars.length) { s = chars[myChar].speed; if (chars[myChar].passive == "speed") s *= 1.15; }
    if (myBoost) s *= 1.6; if (myInside) s *= 0.85;
    return s;
  }

  void _sendState() { lastStateSend = nowSec(); _toHost({"t": "state", "p": {"x": localPos.dx, "y": localPos.dy, "mv": localMoving}}); }

  // ============================= AKSİYONLAR =============================
  void _guardAct(String a) { if (page != 3 || myRole != 0 || winner != 0) return; isHost ? _handleAction(myId, a) : _toHost({"t": "act", "p": {"a": a}}); }

  void _interact() {
    if (page != 3 || myRole != 1 || winner != 0) return;
    String a = _context(); if (a == "none") return;
    isHost ? _handleAction(myId, a) : _toHost({"t": "act", "p": {"a": a}});
  }

  void _ability() { if (page != 3 || myRole != 1 || winner != 0 || myCooldown > 0) return; isHost ? _handleAction(myId, "ability") : _toHost({"t": "act", "p": {"a": "ability"}}); }
  ...
  void _handleAction(int id, String a) {
    ...
    PlayerNet? p = _player(id); if (p == null || !p.alive) return;
    p.lastSeen = nowSec();
    if (id == guardId) { _guardAction(a); return; }
    if (p.role != 1 || p.charId < 0 || p.charId >= chars.length) return;
    CharDef c = chars[p.charId];
    if (a == "ability") { if (gameTime < p.cooldownUntil) return; p.cooldownUntil = gameTime + c.cooldown; _applyActive(p, c); }
  ...
  void _enter(PlayerNet p, int side) {
    ...
    p.insideOffice = true; p.entrySide = side; p.roomId = -2;
    Offset in = side == 0 ? activeMap!.insideLeft : activeMap!.insideRight;
    p.x = in.dx; p.y = in.dy; _burst(in, Colors.red, 14);
  ...
  void _force(PlayerNet p, int side, CharDef c) {
    ...
    double dmg = 25; if (c.passive == "door") dmg *= 1.5;
    _burst(door, Colors.orange, 10); shake = max(shake, 0.5);
    if (side == 0) { dmgL += dmg; if (dmgL >= 100) { dmgL = 0; leftDoorClosed = false; forcedL = gameTime + 5; } }
    else { dmgR += dmg; if (dmgR >= 100) { dmgR = 0; rightDoorClosed = false; forcedR = gameTime + 5; } }
  }
  ...
  void _vent(PlayerNet p, CharDef c) {
    ...
    if (p.ventProgress >= 1) { p.ventProgress = 0; p.insideOffice = true; p.entrySide = 2; p.roomId = -2;
      p.x = activeMap!.insideVent.dx; p.y = activeMap!.insideVent.dy; _burst(activeMap!.insideVent, Colors.yellow, 12);
  ...
  void _attack(PlayerNet p, CharDef c) {
    ...
    if (repelled) {
      ...
      p.insideOffice = false; p.ventProgress = 0; p.x = out.dx; p.y = out.dy; p.roomId = _roomAt(out);
      p.cooldownUntil = max(p.cooldownUntil, gameTime + 2); _burst(out, Colors.blue, 12);
      if (p.id == myId) { myInside = false; localPos = out; }
    } else {
      jumpscare = 1.2; shake = 1.5; _burst(Offset(p.x, p.y), Colors.red, 40);
  ...
  Offset _outside(int side) {
    ...
    if (side == 0) return activeMap!.leftDoor; if (side == 1) return activeMap!.rightDoor; return activeMap!.vent;
  }

  void _applyActive(PlayerNet p, CharDef c) {
    if (activeMap == null) return;
    switch (c.active) {
      case "speed": p.boostUntil = gameTime + 4; break;
      case "dash": Offset d = activeMap!.office.center - Offset(p.x, p.y);
        if (d.distance > 0.1) { Offset t = Offset(p.x, p.y) + (d / d.distance) * 4; if (_walkableFor(t, p)) { p.x = t.dx; p.y = t.dy; _burst(t, c.color, 10); if (p.id == myId) localPos = t; } } break;
      case "camjam": camJam = gameTime + 5; break;
      case "doorbreak": if ((Offset(p.x, p.y) - activeMap!.leftDoor).distance < 3.5) { leftDoorClosed = false; forcedL = gameTime + 5; _burst(activeMap!.leftDoor, Colors.orange, 16); }
        else if ((Offset(p.x, p.y) - activeMap!.rightDoor).distance < 3.5) { rightDoorClosed = false; forcedR = gameTime + 5; _burst(activeMap!.rightDoor, Colors.orange, 16); } break;
      case "ventrush": if ((Offset(p.x, p.y) - activeMap!.vent).distance < 3.5) { p.insideOffice = true; p.entrySide = 2; p.roomId = -2; p.x = activeMap!.insideVent.dx; p.y = activeMap!.insideVent.dy; if (p.id == myId) { myInside = true; localPos = Offset(p.x, p.y); } } break;
      case "drain": if ((Offset(p.x, p.y) - activeMap!.office.center).distance < 10) energy = max(0, energy - 10); break;
      case "lightimmune": p.lightUntil = gameTime + 5; break;
      case "silent": p.silentUntil = gameTime + 5; break;
      case "noise": if (activeMap!.rooms.isNotEmpty) { noiseRoom = rnd.nextInt(activeMap!.rooms.length); noiseUntil = gameTime + 3; } break;
      case "fear": ctrlLock = gameTime + 3; break;
    }
  }
  ...
  void _broadcastSnap() {
    ...
    for (var p in players) { if (!p.alive) continue;
      bool hid = false;
      if (p.role == 1 && p.charId >= 0 && p.charId < chars.length) { if (p.silentUntil > gameTime) hid = true; String pa = chars[p.charId].passive; if (pa == "quiet" || pa == "ghost") hid = true; }
    ...
    for (var a in ais) ents.add({"id": a.id, "ch": a.charId, "rl": 2, "x": a.x, "y": a.y, "rm": a.roomId, "mv": a.moving, "io": false, "cd": 0.0, "hid": false, "sp": false});
  ...
  void _applySnap(Map<String, dynamic> p) {
    if (page != 3) return;
    gameTime = gd(p["t"]); energy = gd(p["e"]); leftDoorClosed = gb(p["ld"]); rightDoorClosed = gb(p["rd"]);
    flashOn = gb(p["fl"]); camOn = gb(p["cam"]); camJam = gameTime + gd(p["jam"]); blackout = gb(p["black"]);
    guardId = gi(p["g"]); noiseRoom = gi(p["nr"]); noiseUntil = gd(p["nu"]);
    if (gi(p["w"]) > 0) { _endLocal(gi(p["w"]), gs(p["m"])); return; }
    remote.clear();
    if (p["ents"] is List) for (var it in (p["ents"] as List)) {
      if (it is Map) { Map<String, dynamic> e = it.cast<String, dynamic>(); int id = gi(e["id"]);
        if (id == myId) { myInside = gb(e["io"]); myCooldown = gd(e["cd"]); myBoost = gb(e["sp"]);
          Offset hp = Offset(gd(e["x"]), gd(e["y"])); if ((localPos - hp).distance > 5) localPos = hp; continue; }
  ...
  void _endGame(int w, String m) {
    ...
    winner = w; _toAll({"t": "over", "p": {"w": w, "m": m}}); _endLocal(w, m);
  }

  void _endLocal(int w, String m) {
    if (page == 4) return;
    winner = w; endMsg = m; page = 4; _ticker?.dispose(); _ticker = null; ui();
  }

  void _removePlayer(int id) {
    if (id == 0) return;
    if (page == 3 && id == guardId) { players.removeWhere((p) => p.id == id); endpoints.remove(id); _endGame(2, "Güvenlik ayrıldı."); return; }
    players.removeWhere((p) => p.id == id); endpoints.remove(id);
  ...
  void _burst(Offset pos, Color color, int count) {
    ...
    for (int i = 0; i < n; i++) {
      double ang = rnd.nextDouble() * 2 * pi; double sp = 1 + rnd.nextDouble() * 4;
  ...
  // ============================= GEOMETRİ =============================
  PlayerNet? _player(int id) { for (var p in players) if (p.id == id) return p; return null; }
  ...
  int _freeChar(List<int> used) { for (int i = 0; i < 100; i++) { int c = rnd.nextInt(chars.length); if (!used.contains(c)) return c; } for (int c = 0; c < chars.length; c++) if (!used.contains(c)) return c; return 0; }
  ...
  List<EntityView> _views() {
    ...
    if (isHost) {
      for (var p in players) { if (!p.alive) continue;
        bool hid = false; if (p.role == 1 && p.charId >= 0 && p.charId < chars.length) { if (p.silentUntil > gameTime) hid = true; String pa = chars[p.charId].passive; if (pa == "quiet" || pa == "ghost") hid = true; }
  ...
  String _detected() {
    ...
    List<String> names = [];
    for (var v in _views()) if (v.role != 0 && !v.inside && v.room == selectedCamRoom && v.moving && !v.hidden) names.add(_charName(v.charId));
    String t = "";
  ...
  String _clock() {
    ...
    if (h >= 6) return "06:00"; if (h <= 0) return "12:00"; return "0$h:00";
  }

  // ============================= UI =============================
  @override
  Widget build(BuildContext context) {
    return Scaffold(backgroundColor: const Color(0xFF050508), body: SafeArea(child: Builder(builder: (_) {
  ...
  Widget _menu() => Padding(padding: const EdgeInsets.all(20), child: Column(children: [
    ...
    const SizedBox(height: 12), Text(status, textAlign: TextAlign.center, style: const TextStyle(fontSize: 15)),
    const SizedBox(height: 20),
    _btn("HOST OL", hostGame), _btn("OYUN BUL", openDiscovery),
    ...
    const SizedBox(height: 10), _btn("IP İLE KATIL", joinIp),
    const SizedBox(height: 10), _btn("GRAFİK AYARLARI", () { _showSettings = true; ui(); }),
  ...
  Widget _lobby() {
    ...
    return Padding(padding: const EdgeInsets.all(16), child: Column(children: [
      ...
      const SizedBox(height: 6), Text(pt, textAlign: TextAlign.center),
      const SizedBox(height: 10), Text("Harita: ${makeMap(currentMap).name}"),
      Row(children: [
      ...
      Expanded(child: ListView.builder(itemCount: chars.length, itemBuilder: (c, i) {
        ...
        Color bg = Colors.grey[850]!; if (me) bg = Colors.green[800]!; if (other) bg = Colors.red[900]!;
  ...
  Widget _game() {
    ...
    if (blackout) top += " | KARANLIK!"; if (camJam > gameTime) top += " | PARAZİT!";
    if (views.any((v) => v.role == 1 && v.inside)) top += " | OFİSTE BİRİ VAR!";
    return Stack(children: [
      ValueListenableBuilder<int>(valueListenable: _frame, builder: (c, f, w) => Positioned.fill(child: CustomPaint(painter: GamePainter(map: activeMap, entities: views, q: q, particles: _particlesOn ? particles : const [], shake: shake, jumpscare: jumpscare, myRole: myRole, localPos: localPos, flashOn: flashOn, blackout: blackout, leftClosed: leftDoorClosed, rightClosed: rightDoorClosed, camActive: camActive && myRole == 0, rnd: rnd, gameTime: gameTime)))),
  ...
  Widget _camOverlay() {
    ...
    return Positioned(top: 50, left: 8, right: 8, height: 210, child: Container(padding: const EdgeInsets.all(8), color: const Color(0xDD001400), child: Column(children: [
      ...
      const SizedBox(height: 4), Expanded(child: Text(_detected(), style: const TextStyle(color: Color(0xFF66FF66)))),
  ...
  Widget _animControls() {
    ...
    if (a == "attack") label = "SALDIR!"; else if (a == "enterL") label = "SOLDAN SIZ"; else if (a == "enterR") label = "SAĞDAN SIZ";
    else if (a == "forceL") label = "SOL ZORLA"; else if (a == "forceR") label = "SAĞ ZORLA"; else if (a == "vent") label = "HAVALANDIRMA";
    return Stack(children: [
  ...
  Widget _joystick() => GestureDetector(
    onPanStart: (d) => _joy(d.localPosition), onPanUpdate: (d) => _joy(d.localPosition),
  ...
  void _joy(Offset local) {
    Offset v = local - const Offset(75, 75); double d = v.distance;
    if (d > 60) { v = (v / d) * 60; }
    joyThumb = v; joyX = v.dx / 60; joyY = v.dy / 60;
  }
  ...
  Widget _end() {
    ...
    return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 12), Text(per, style: const TextStyle(fontSize: 20)),
      const SizedBox(height: 12), Text(endMsg, style: const TextStyle(color: Colors.white60)),
      const SizedBox(height: 24), _btn("ANA MENÜ", returnMenu),
    ])));
  }
...
class GamePainter extends CustomPainter {
  final MapDef? map; final List<EntityView> entities; final GQ q; final List<Particle> particles;
  final double shake, jumpscare, gameTime; final int myRole; final Offset localPos;
  final bool flashOn, blackout, leftClosed, rightClosed, camActive; final Random rnd;

  GamePainter({required this.map, required this.entities, required this.q, required this.particles,
    required this.shake, required this.jumpscare, required this.myRole, required this.localPos,
    required this.flashOn, required this.blackout, required this.leftClosed, required this.rightClosed,
    required this.camActive, required this.rnd, required this.gameTime});
  ...
  void paint(Canvas c, Size size) {
    ...
    double sx = 0, sy = 0;
    if (q.shake && shake > 0) { sx = (rnd.nextDouble() - 0.5) * shake * 10; sy = (rnd.nextDouble() - 0.5) * shake * 10; }
    ...
    for (var room in map!.rooms) {
      ...
      } else c.drawRect(r, Paint()..color = const Color(0xFF141220));
    ...
    // Kapılar (kayan)
    _door(c, s(map!.leftDoor), scale, leftClosed, true);
    _door(c, s(map!.rightDoor), scale, rightClosed, false);
    c.drawCircle(s(map!.vent), 5 * (scale / 1), Paint()..color = Colors.yellowAccent);

    // Varlıklar
    for (var e in entities) _entity(c, s(Offset(e.x, e.y)), e, scale);
    ...
    if (q.darkness) {
      ...
      if (!blackout) c.drawCircle(s(map!.office.center), 60, hole..shader = RadialGradient(colors: [Colors.white, Colors.transparent]).createShader(Rect.fromCircle(center: s(map!.office.center), radius: 60)));
      if (flashOn && !blackout) c.drawCircle(s(map!.office.center), 110, hole..shader = RadialGradient(colors: [Colors.white, Colors.transparent]).createShader(Rect.fromCircle(center: s(map!.office.center), radius: 110)));
      if (myRole == 1) c.drawCircle(s(localPos), 70, hole..shader = RadialGradient(colors: [Colors.white, Colors.transparent]).createShader(Rect.fromCircle(center: s(localPos), radius: 70)));
      c.restore();
    }
    ...
    // Jumpscare
    if (jumpscare > 0) {
      double a = jumpscare.clamp(0.0, 1.0);
  ...
  void _door(Canvas c, Offset p, double scale, bool closed, bool left) {
    ...
    double w = 8.0, h = closed ? 14.0 : 5.0;
    c.drawRect(Rect.fromCenter(center: p, width: w, height: h), Paint()..color = col);
  }

  void _entity(Canvas c, Offset p, EntityView e, double scale) {
