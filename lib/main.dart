...
String actDesc(String a) {
  switch (a) {
    ...
    case 'scream':
      return 'Guvenligi 5 sn kilitler ve sarsar';
    case 'dash':
      return 'Merkeze dogru atilma';
    case 'teleport':
      return 'Rastgele bir odaya isinlanir';
    case 'sabotage':
      return 'Uzaktan 6 guc emer';
    case 'drain':
...
List<CD> mkChars() {
  return [
    ...
    CD('Pas', 0xFF8D6E63, 2.8, 15, 'doorbreak', 'vent',
        'Depoda unutulan en eski prototip. Eklemleri pasli ama yumrugu hala celik gibi.'),
    CD('Sis', 0xFF90A4AE, 3.3, 13, 'teleport', 'quiet',
        'Yangin sondurme sistemine karisan buhar robotu. Bir odada belirir, digerinde kaybolur.'),
    CD('Kukla', 0xFFB39DDB, 3.1, 18, 'scream', 'ghost',
        'Muzik kutusunun icindeki asil kukla. Ciglik attiginda tum vardiya donar.'),
    CD('Anten', 0xFF4FC3F7, 3.0, 12, 'sabotage', 'focus',
        'Catidaki verici robotu. Elektrik sebekesine uzaktan sizip gucu kemirir.'),
    CD('Kibrit', 0xFFFF8A65, 3.4, 12, 'lightimmune', 'speed',
        '1987 yanginini baslatan kivilcimin sahibi. Ates onu bir kez yakti; isik artik dostu.'),
    CD('Buz', 0xFF80DEEA, 2.8, 16, 'fear', 'door',
        'Dondurma standinin devi. Soguk bakisi elleri dondurur, omzu kapilari kirar.'),
    CD('Yilan', 0xFFAED581, 3.5, 12, 'ventrush', 'quiet',
        'Boru sisteminin yeni sahibi. Sessizce surunur, vente goz acip kapayana kadar girer.'),
    CD('Radyo', 0xFFFFF176, 3.2, 10, 'noise', 'focus',
        'Eski anons sistemi. Her kamerada baska bir odadan geliyormus gibi konusur.'),
    CD('Makas', 0xFFF48FB1, 3.3, 11, 'dash', 'light',
        'Kostum atolyesinin terzisi. Fener uzerine tutuldugunda coktan baska yerdedir.'),
    CD('Son', 0xFFE0E0E0, 3.0, 20, 'teleport', 'ghost',
        'Yangindan sag cikan tek sey. Kimse onu kamerada gormedi; gorenler anlatamadi.'),
  ];
}
...
MD mkMap(int i) {
  ...
  if (i == 2) {
    ...
    return MD('Sahne', of, const Offset(-24, 0), const Offset(24, 0),
        const Offset(0, -20), const Offset(-16, 0), const Offset(16, 0),
        const Offset(0, -11), [
  ...
  if (i == 3) {
    ...
    return MD('Bodrum', of, const Offset(-20, 0), const Offset(20, 0),
        const Offset(0, -16), const Offset(-13, 0), const Offset(13, 0),
        const Offset(0, -9), [
...
class _S extends State<GS> {
  ...
  void act(int id, String a) {
    ...
    if (a == 'ability') {
      if (gt < p.cd) return;
      final double cdv = c.pas == 'focus' ? c.cd * 0.75 : c.cd;
      p.cd = gt + cdv;
      applyA(p, c);
    } else if (a == 'enterL') {
      ent(p, 0);
  ...
  void applyA(PN p, CD c) {
    ...
    if (c.act == 'speed') {
      ...
      lock = gt + 3;
    } else if (c.act == 'scream') {
      lock = gt + 5;
      shake = max(shake, 1.0);
    } else if (c.act == 'teleport') {
      if (!p.inside && map!.rooms.isNotEmpty) {
        final r = map!.rooms[rnd.nextInt(map!.rooms.length)];
        final t = r.r.center;
        p.x = t.dx;
        p.y = t.dy;
        p.room = roomAt(t);
  ...
  Widget _page2() {
    ...
    return ListView(
      ...
      children: [
        Text('LOBI - ${mkMap(curMap).n.toUpperCase()}',
            style: const TextStyle(
                color: Colors.red,
                fontFamily: 'monospace',
                fontWeight: FontWeight.bold,
        ...
        if (isHost)
          Wrap(spacing: 8, runSpacing: 8, children: [
            for (int mi = 0; mi < 4; mi++)
              SizedBox(
                width: 150,
                child: _fnafButton(mkMap(mi).n.toUpperCase(),
                    curMap == mi ? Colors.green : Colors.amber,
                    () => setM(mi)),
              ),
          ]),
        const SizedBox(height: 10),
